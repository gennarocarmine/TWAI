...
\end{enumerate}
\noindent
\textbf{Nota sul Bilanciamento:} Poiché il gioco del Forza 4 tende a favorire il primo giocatore, il dataset grezzo presentava un leggero sbilanciamento verso la classe 1. In fase di post-processing, abbiamo applicato un sottocampionamento (\textit{undersampling}) della classe maggioritaria per ottenere una distribuzione finale bilanciata ($\approx 33\%$ per classe).
...
\end{itemize}
\noindent
Il valore di un nodo (stato di gioco) è calcolato retropagando i valori dai nodi foglia fino alla radice. Tuttavia, la complessità temporale di base O($b^d$) l'algoritmo impraticabile per profondità elevate. Per ottimizzare la ricerca, abbiamo implementato la potatura \textbf{Alpha-Beta Pruning}.\\\\
\noindent
Questa tecnica mantiene due parametri, $\alpha$ (miglior valore per MAX) e $\beta$ (miglior valore per MIN). Se durante l'esplorazione si trova una mossa che peggiora la situazione rispetto a quanto già garantito dai rami precedenti, l'intero sotto-albero viene tagliato (pruned). Ciò riduce la complessità temporale nel caso medio da $O(b^d)$ a $O(b^{d/2})$, permettendo all'agente di esplorare una profondità doppia rispetto al Minimax puronello stesso intervallo di tempo.

\subsubsection{Ottimizzazione: Ordinamento delle Mosse (Move Ordering)}
L'efficacia della potatura Alpha-Beta dipende fortemente dall'ordine in cui le mosse vengono valutate. Nel caso peggiore, ovvero in cui le mosse migliori venissero esaminate per ultime, non avrebbe alcun taglio el'algoritmo degenererebbe in un Minimax puro, rendendo inutile l'ottimizzazione.\\\\
...
\begin{enumerate}
    \item \textbf{Input Layer (42 Neuroni):} Riceve il vettore linearizzato della scacchiera (6 x 7), normalizzando lo stato di gioco.
    ...
    \item \textbf{Activation Function (ReLU):} Utilizziamo la \textit{Rectified Linear Unit} ($f(x) = \max(0, x)$) per gli strati nascosti. Questa funzione è preferita alla sigmoide per le reti profonde in quanto mitiga il problema del \textit{vanishing gradient} e, grazie alla sua semplicità computazionale, favorisce una convergenza più rapida durante l'addestramento.
